#!/usr/bin/env python3
"""Descarga 110 fotos 15k-20k, redimensiona a 1400x320 y guarda <=250KB."""
import io
import json
import sys
import urllib.parse
...
TARGET_WIDTH = 1400; TARGET_HEIGHT = 320; TARGET_KB = 250
...
def process(slug, url):
    ...
    if out.exists() and out.stat().st_size <= TARGET_KB * 1024 and out.stat().st_size > 5000:
...
def main():
    ...
    photos = json.loads((ROOT / "data" / "city_photos_15k_20k.json").read_text(encoding="utf-8"))
    ok = err = 0
    for slug, info in photos.items():
        ...
        try:
            ...
            err += 1
    print(f"\n=== OK={ok} ERR={err} TOTAL={len(photos)} ===")
