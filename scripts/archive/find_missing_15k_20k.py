#!/usr/bin/env python3
"""Lista municipios espanoles 15.000-20.000 habitantes SIN pagina rentabilidad-*.html."""
import json
import re
import sys
import unicodedata
...
def main():
    ...
    for r in ws.iter_rows(min_row=3, values_only=True):
        ...
        if not cpro or not nombre or pob is None:
            continue
        try:
            pob = int(pob)
        except (ValueError, TypeError):
            continue
        if pob < 15000 or pob >= 20000:
            continue
        ccaa = CPRO_CCAA.get(str(cpro).zfill(2), "??")
        rows.append((nombre, pob, prov, ccaa, cpro))
    ...
    print(f"Total municipios 15k-20k habitantes: {len(rows)}")
    ...
    (ROOT / "data" / "missing_15k_20k.json").write_text(
        json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    for i, c in enumerate(out, 1):
        print(f"{i:>3}  {c['slug']:<35} {c['nombre']:<35} {c['poblacion']:>7,}  {c['ccaa']}  {c['provincia']}")
