#!/usr/bin/env python3
"""Descarga las 100 fotos seleccionadas, redimensiona a 1400x320 y guarda <=250KB."""
import io
import json
import sys
import urllib.parse
...
def main():
    ...
    results_path = ROOT / "data" / "city_photos_20k_30k.json"
    photos = json.loads(results_path.read_text(encoding="utf-8"))

    ok = err = 0
    for slug, info in photos.items():
        if info is None:
            print(f"[ERR] {slug}: no photo found", flush=True)
            err += 1
            continue
        try:
            ...
            err += 1
    print(f"\n=== summary: OK={ok} ERR={err} TOTAL={len(photos)} ===")
