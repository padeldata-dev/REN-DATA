#!/usr/bin/env python3
"""Lista municipios espanoles con poblacion 30.000-50.000 que NO tienen pagina rentabilidad-*.html."""
import json
import re
import sys
import unicodedata
...
def candidate_slugs(name):
    ...
    # Aliases conocidos del listado existente
    aliases = {
        "alacant-elx": ["elche"],
...
def main():
    ...
    print(f"Total municipios 30k-50k habitantes: {len(rows)}")
    ...
    (ROOT / "data" / "missing_30k_50k.json").write_text(
        json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print(f"\nJSON guardado en data/missing_30k_50k.json")
